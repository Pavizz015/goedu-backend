...
func lessonsListHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := DB.Query(r.Context(), `
		SELECT id, title, theory, question, options, difficulty, category
		FROM lessons
		ORDER BY id
	`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&l.ID, &l.Title, &l.Theory, &l.Question, &optionsJSON, &l.Difficulty, &l.Category); err != nil {
			http.Error(w, "db error", http.StatusInternalServerError)
			return
		}
...
func lessonGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = DB.QueryRow(r.Context(), `
		...
		FROM lessons
		WHERE id=$1
	`, id).Scan(&l.ID, &l.Title, &l.Theory, &l.Question, &optionsJSON, &l.Difficulty, &l.Category)
	if err != nil {
...
func leaderboardHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := DB.Query(r.Context(), `
		SELECT email, xp, COALESCE(username, '') as username
		FROM users
		ORDER BY xp DESC
		LIMIT 10
	`)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.Email, &row.XP, &row.Username); err != nil {
			continue
		}
		row.Level, row.Title = calcLevel(row.XP)
		out = append(out, row)
	}
